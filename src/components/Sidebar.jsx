...
const Sidebar = ({isMenuOpen, setIsMenuOpen}) => {
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 
      ...
      border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute 
      left-0 z-10 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
      {/* logo */}
      <img
      ...
      <div className="flex-1 overflow-y-scroll mt-3 text-sm space-y-3">
        {chats
          ...
          .map((chat) => (
            <div
              key={chat.id || chat._id || chat.name}
              onClick={() =>{navigate('/');setSelectedChat(chat);setIsMenuOpen(false);} }
              className="w-full mt-2 p-2 px-4 text-left rounded-md border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group"
            >
              <div>
      ...
      >
        ...
        <div className="flex flex-col text-sm">
          ...
          <p className="text-xs text-gray-400">
            Purchase creadits to use quickgpt
          </p>
        </div>
      </div>
